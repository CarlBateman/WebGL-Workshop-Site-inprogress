...
function main() {
  ...
  (function init() {
    ...
    if (gl) {
      ...
      document.getElementById("showAntiCube").onchange = show;
      document.getElementById("showCube").onchange = show;
  ...
  function initGeometry() {
    ...
    initBuffer(new Float32Array(antiCubeGeometry.vertices), bufferInfo[1].vertices);
    initBuffer(new Float32Array(antiCubeGeometry.colors), bufferInfo[1].colors);
    initIndices(new Uint16Array(antiCubeGeometry.indices), bufferInfo[1].indices);
  }
  ...
  function initIndices(indices, buffer) {
    buffer.numItems = cubeGeometry.indices.length / buffer.itemSize;
    buffer.id = gl.createBuffer();
  ...
  function draw() {
    ...
    //gl.drawArrays(gl.TRIANGLES, 0, bufferInfo[0].numItems);
    //gl.drawElements(gl.LINE_STRIP, bufferInfo[0].indices.numItems, gl.UNSIGNED_SHORT, 0);
