...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var i = 0, x = -edge.x, y = -edge.y, z = -edge.y;
  for (var ix = 0; ix < totSegs.x; ix++) {
    for (var iy = 0; ix < totSegs.y; iy++) {
      for (var iz = 0; iz < totSegs.z; iz++) {
        var v = geometry.vertices[i];
        v.z = z;
        if (iz < numCurveSegs || iz > numCurveSegs + numSegs) {
          z += outerStepSize;
        } else {
          z += innerStepSize.z;
        }
